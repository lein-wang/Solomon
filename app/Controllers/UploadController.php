...
class UploadController extends Controller
{
    ...
    public function upload($request, $response, $args)
    {
        ...
        $uploadUrl = $http_type . $_SERVER['HTTP_HOST'] . '/static/upload';//返回的路径名
        //文件保存目录路径
        $save_root = RES_PATH . '/static/upload/';
...
//        $save_root = realpath($save_root) . '/';
        ...
        foreach ($willArr as $will) {
            ...
            //是否裁剪
            $cut = isset($_GET['x']) ? $_GET['x'] : (isset($_POST['x']) ? $_POST['x'] : '600x600i100x100');
            ...
            if (isset($file_cuted) && $file_cuted) {
                ...
                if ($file_ext === 'mp4') {
                    //output video name
                    $video_thumb_name = CFun::crcU32($tmp_name) . ".jpg";
...
//         print_r($resultArr);die;
    ...
    {
        $uploadLoc = $this->getStaticLocation(CCommon::$rand_folder);//上传到服务器的位置
