    ...
    $.fn.extend({
        Anyupload: function(options){
            ...
            uploaders.each(function(){
                ...
                var _tpl    = tpl_upload.replace(/\-\-iframename\-\-/g, target).replace('--attr--',attr);
                ...
                if(defaults){
                    try{
                        ...
                        for(var i=0,length=trr.length; i<length; i++){
                            ...
                            if(_r[0].length > 0){
                                dft_urls.push({name:_r[0],url:_r[1]});
                            }
                        }
                ...
                upbox.find("iframe[name='"+target+"']").on("load", function(){
                    ...
                    // eval("var json = " + jQuery.parseJSON($(doc).find("body").text().replace(/<embed.*/g, '')));
                    var json = jQuery.parseJSON($(doc).find("body").text().replace(/<embed.*/g, ''));
