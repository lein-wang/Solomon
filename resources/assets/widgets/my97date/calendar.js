...
My97DP.prototype = {
    ...
    update: function($) {
        ...
        if ($dp.el[$dp.elProp] != $) {
            var old = $dp.el[$dp.elProp];
            var val = old.replace(/\d{1,4}\-\d{1,2}\-\d{1,2}/, $); 
            // console.log(old, '-', $, '-', val);
            if(val.length < 10) {
                val = $;
            }
    ...
    _fd: function() {
        ...
        if (A > 1) A -= 7;
        var now_time = this.formatTime();
        H.a("<table width=100% border=0 cellspacing=0 cellpadding='3px'><tr><td align='center' class='Wday' onmouseout='this.className=\"Wday\"' onmouseover='this.className=\"WdayOn\"' onclick='time_Click(\"00:00:00\");'>00:00:00</td><td align='center' class='Wday' onmouseout='this.className=\"Wday\"' onmouseover='this.className=\"WdayOn\"' onclick='time_Click(\"--now-time--\");'>--now-time--</td><td align='center' class='Wday' onmouseout='this.className=\"Wday\"' onmouseover='this.className=\"WdayOn\"' onclick='time_Click(\"23:59:59\");'>23:59:59</td></tr></table>".replace(/--now-time--/g,now_time));
    ...
    _fillQS: function(C, A) {
        ...
        E = B.style,
        _ = new sb();
        _.a("<table class=WdayTable style='border:1px solid #eee;' width=100% height=100% border=0 cellspacing=0 cellpadding=0>");
        _.a("<tr class=MTitle><td><div style=\"float:left\"></div>");
        if (!C) _.a("<div style=\"float:right;cursor:pointer;padding:0 8px;\" onclick=\"hide($d.qsDivSel);\">\xd7</div>");
        _.a("</td></tr>");
    ...
    pickTime: function(str_time) {
        // $dt.d = Math.min(new Date($dt.y, $dt.M, 0).getDate(), $dt.d);
        // this.update();
        var old = $dp.el.value;
        old = old.replace(/\d{1,2}\:\d{1,2}\:\d{1,2}/gi,'').replace(/^\s*/g,'').replace(/\s*$/g,'');
        // old = old.replace(/\d{1,4}\-\d{1,2}\-\d{1,2}/, old); 
        // if(old.length < 9) {
        //     formated += ' ' + old;
        // }else {
        //     formated = old;
        // }
        var val = old + ' ' + str_time;
